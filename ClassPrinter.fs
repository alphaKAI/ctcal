...
type class_ = Unregistered | Registered of class_entry

let classes = [
  [Unregistered; Unregistered; Unregistered; Registered {id="GB11514"; name="シミュレーション物理"; teacher="狩野 均 "}; Unregistered];
  [Unregistered; Unregistered; Unregistered; Registered {id="GB11514"; name="シミュレーション物理"; teacher="狩野 均 "}; Unregistered];
  [Registered {id="GB11954"; name="システムプログラミング序論"; teacher="大山 恵弘"}; Registered {id="GB12501"; name="情報理論"; teacher="伊藤誠"}; Unregistered; Unregistered; Registered {id="GB12501"; name="情報理論"; teacher="伊藤誠"}];
  [Registered {id="GB11954"; name="システムプログラミング序論"; teacher="大山 恵弘"}; Registered {id="GB12501"; name="情報理論"; teacher="伊藤誠"}; Registered {id="31FM012";name="English Intercultural Communication II";teacher="ロリエ, ジョエル "}; Unregistered; Registered {id="GB12501"; name="情報理論"; teacher="伊藤誠"}];
  [Unregistered; Unregistered; Unregistered; Unregistered; Unregistered];
  [Unregistered; Registered {id="31GM012"; name="English Integrated Skills II";teacher="サラザール, ハビエル"}; Unregistered; Unregistered; Unregistered];
]
...
let unregisterd = {id="未登録"; name=""; teacher=""}
...
let padding_list_with_impl append list len pad =
  if List.length list < len then
    if append then
      List.append list (MyUtil.range 0 (len - List.length list - 1) |> List.map (fun _ -> pad))
    else
      List.append (MyUtil.range 0 (len - List.length list - 1) |> List.map (fun _ -> pad)) list
  else
    list

let padding_list_with = padding_list_with_impl true

let padding_list_with_prepend: string list -> int -> string -> string list = padding_list_with_impl false
...
let tarnsform_crass_entry_to_vis max_width max_name_lines =
  ...
  | Unregistered ->
    ...
    {
      id = uv.id;
      names = padding_list_with uv.names max_name_lines (PrintUtil.str_repeat " " max_width);
      teacher = PrintUtil.str_repeat " " max_width
    }
  | Registered class_ ->
    let splits =
      let rec splits_impl ret i j text max_width =
        if String.length text <= i then
          ret |> List.rev
        else begin
          ...
          if Unicode.east_asian_width sub_str >= max_width - 1 || text.[i] = ' '  then
            if Unicode.east_asian_width sub_str <> String.length sub_str then
              ...
              splits_impl (i :: ret) (i + 1) i text max_width
          else
            splits_impl ret (i + 1) j text max_width
        end
      splits_impl [] 0 0

    let (_, names) = List.append (splits class_.name max_width) [0]
...
let chmax rv v = if !rv < v then rv := v
...
let print_classes (classes: class_ list list) max_width =
  ...
  let max_name_lines =
    ...
    for i = 0 to 29 do
      let x = i % 6
      let y = i / 6
      let c = classes.[x].[y] |> function | Unregistered -> unregisterd | Registered class_ -> class_
      let name = c.name
      chmax max_name_lines (max (Unicode.east_asian_width name / max_width + 1) (String.count name (fun c -> c = ' ')))
    !max_name_lines

  List.map (fun sub_List ->
    List.map (fun class_ -> tarnsform_crass_entry_to_vis max_width max_name_lines class_) sub_List) classes
  |> List.iter (fun weekly_classes ->
      ...
      MyUtil.range 0 (max_name_lines - 1)
      |> List.iter (fun i ->
          let t_names = List.map (fun names -> List.item i names) nameses
