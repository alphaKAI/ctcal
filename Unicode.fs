...
let kHighSurrogateMask = (1 <<< kSurrogateBits) - 1

let kLowSurrogateMin = 0xDC00
let kLowSurrogateMax = 0xDFFF
let kLowSurrogateMask = (1 <<< kSurrogateBits) - 1

let kSurrogateMin = kHighSurrogateMin
let kSurrogateMax = kLowSurrogateMax
let kSurrogateMask = (1 <<< (kSurrogateBits + 1)) - 1

let kHighSurrogateOffset = kHighSurrogateMin - (0x10000 >>> 10)
  ...
  let rec binary_range_search_impl heads tails value head tail =
    if head > tail then
      if tail < 0 || List.length tails - 1 < tail then
        ...
        value <= List.item tail tails
    else
      let where = (float(head + tail)) / 2. |> floor |> int
      let v = List.item where heads
      if value = v then
        true
      else if value < v then
        binary_range_search_impl heads tails value head (where - 1)
      else
        binary_range_search_impl heads tails value (where + 1) tail
  let head = 0
  let tail = List.length heads - 1
  binary_range_search_impl heads tails value head tail

let isHighSurrogate uc = (uc &&& (~~~ kHighSurrogateMask)) = kHighSurrogateMin

let isLowSurrogate uc = (uc &&& (~~~ kLowSurrogateMask)) = kLowSurrogateMin

let isSurrogate uc = (uc &&& (~~~ kSurrogateMask)) = kSurrogateMin

let decodeSurrogatePair high low = ((high &&& kHighSurrogateMask) <<< kSurrogateBits) + (low &&& kLowSurrogateMask) + 0x10000

let charCodeAt (str : string) i = int(str.[i])

exception UTF16DecodeException

let east_asian_width text =
  let rec east_asian_width_impl text width i =
    if i >= String.length text then
      width
    else
      let proc_for_next code i =
        if binary_range_search start_group end_group code then
          east_asian_width_impl text (width + 2) (i + 1)
        else
          east_asian_width_impl text (width + 1) (i + 1)

      let code = charCodeAt text i
      if isSurrogate code then
        if not (isHighSurrogate code) then
          raise UTF16DecodeException
        else
          let i = i + 1
          let low_code = charCodeAt text i
          if not (isLowSurrogate low_code) then
            raise UTF16DecodeException
          else
            let code = decodeSurrogatePair code low_code
